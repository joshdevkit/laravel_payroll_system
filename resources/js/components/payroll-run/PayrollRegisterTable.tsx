...
import { num, peso, totalDeductions } from "./payrollRunUtils";
...
export function PayrollRegisterTable({ items }: Props) {
    ...
    const footerTotalEarnings = sum(totalEarnings);

    const totalDeductionsForEmployee = (item: PayrollItem) =>
        Number(item.tardy_deduction ?? 0) +
            ...
            <table className="w-full border-collapse text-[11px] leading-tight">
                ...
                <tfoot>
                    <tr className="bg-amber-200 text-center font-semibold text-black dark:bg-amber-500/15 dark:text-amber-100">
                        ...
                        <td className={td}>
                            {sum((i) => Number(i.present_days ?? 0))}
                        </td>
                        <td className={td}>-</td>
                        <td className={td}>
                            {num(sum((i) => Number(i.basic_pay ?? 0)))}
                        </td>
                        <td className={td}>
                            {num(sum((i) => Number(i.tardy_deduction ?? 0)))}
                        </td>
                        <td className={td}>{footerTotalEarnings}</td>
                        <td className={td}>{num(sum(colaOf))}</td>
                        <td className={td}>
                            {num(sum((i) => Number(i.overtime_pay ?? 0)))}
                        </td>
                        <td className={td}>
                            {num(sum((i) => Number(i.holiday_pay ?? 0)))}
                        </td>
                        <td className={td}>
                            {num(sum((i) => Number(i.night_diff ?? 0)))}
                        </td>
                        <td className={td}>
                            {num(
                                sum(
                                    (i) =>
                                        Number(i.total_earnings ?? 0) +
                                        colaOf(i),
                                ),
                            )}
                        </td>
                        <td className={td}>
                            {num(
                                sum((i) => Number(i.philhealth_deduction ?? 0)),
                            )}
                        </td>
                        <td className={td}>
                            {num(sum((i) => Number(i.pagibig_deduction ?? 0)))}
                        </td>
                        <td className={td}>
                            {num(sum((i) => Number(i.sss_deduction ?? 0)))}
                        </td>
                        <td className={td}>-</td>
                        <td className={td}>-</td>
                        <td className={td}>-</td>
                            ...
                            {num(
                                sum(
                                    (i) =>
                                        ...
                                        Number(i.tax_deduction ?? 0) +
                                        Number(i.leave_deduction ?? 0),
                                ),
                            )}
                        </td>
                        <td className={td}>
