...
import { CalendarClock, ChevronLeft, ChevronRight, Pencil, Plus, RefreshCw, Trash2 } from 'lucide-react';
...
const PAGE_SIZE = 15;
const emptyForm: HolidayForm = { date: '', name: '', type: 'regular', notes: null };
const typeLabel: Record<HolidayType, string> = {
    regular: 'Regular Holiday',
    special_non_working: 'Special Non-Working Day',
};

export default function Holidays({ holidays, currentYear }: { holidays: Holiday[]; currentYear: number }) {
    ...
    const years = useMemo(() => Array.from({ length: 5 }, (_, index) => currentYear - 2 + index), [currentYear]);
    ...
    const openEdit = (holiday: Holiday) => {
        setEditing(holiday);
        setForm({ date: holiday.date, name: holiday.name, type: holiday.type, notes: holiday.notes });
        setFormOpen(true);
    };
    ...
    const save = () => {
        ...
        const options = {
            preserveScroll: true,
            onSuccess: () => { setFormOpen(false); setPage(1); },
    ...
    const remove = () => {
        ...
        router.delete(`/holidays/${deleting.id}`, {
            preserveScroll: true,
            onSuccess: () => { setDeleteOpen(false); setDeleting(null); },
            onFinish: () => setBusy(false),
        });
    };
    ...
    const sync = () => {
        setBusy(true);
        router.post('/holidays/sync', { year: syncYear }, {
            preserveScroll: true,
            onSuccess: () => setPage(1),
            onFinish: () => setBusy(false),
        });
            ...
            <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        ...
                        <h1 className="mt-1 font-display text-2xl font-bold sm:text-3xl">Holidays</h1>
                        <p className="mt-2 text-sm text-muted-foreground">Manage holidays used by attendance and payroll calculations.</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        <select aria-label="Holiday sync year" value={syncYear} onChange={(e) => setSyncYear(Number(e.target.value))} disabled={busy} className="h-9 rounded-md border bg-background px-3 text-sm">
                            {years.map((year) => <option key={year} value={year}>{year}{year === currentYear ? ' (Current)' : ''}</option>)}
                        </select>
                        <Button variant="outline" onClick={sync} disabled={busy}>
                            <RefreshCw className={`mr-2 h-4 w-4 ${busy ? 'animate-spin' : ''}`} />
                            {busy ? 'Working…' : `Sync ${syncYear}`}
                        </Button>
                        <Button onClick={openCreate} disabled={busy}><Plus className="mr-2 h-4 w-4" />Add Holiday</Button>
                    </div>
                </div>

                <div className="mt-6 flex items-center justify-between gap-3 text-sm text-muted-foreground">
                    <span>{holidays.length} holiday{holidays.length === 1 ? '' : 's'} loaded</span>
                    {holidays.length > 0 && <span>{Array.from(new Set(holidays.map((holiday) => holiday.date.slice(0, 4)))).sort().join(', ')}</span>}
                </div>
                ...
                <div className="mt-3 overflow-hidden rounded-lg border">
                    {holidays.length === 0 ? (
                        <div className="flex flex-col items-center gap-2 p-12 text-center">
                            ...
                            <p className="max-w-md text-sm text-muted-foreground">Select a year and sync the Philippine holiday calendar, or add a custom holiday.</p>
                        </div>
                    ) : (
                        <>
                            <Table>
                                <TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Holiday</TableHead><TableHead>Type</TableHead><TableHead>Notes</TableHead><TableHead className="w-[120px] text-right">Actions</TableHead></TableRow></TableHeader>
                                <TableBody>
                                    {paginated.map((holiday) => (
                                        <TableRow key={holiday.id}>
                                            <TableCell className="font-medium">{new Date(`${holiday.date}T00:00:00`).toLocaleDateString('en-PH', { year: 'numeric', month: 'short', day: 'numeric' })}</TableCell>
                                            <TableCell>{holiday.name}</TableCell>
                                            <TableCell><Badge variant="outline">{typeLabel[holiday.type]}</Badge></TableCell>
                                            <TableCell>{holiday.notes || '—'}</TableCell>
                                            <TableCell><div className="flex justify-end gap-1">
                                                <Button variant="ghost" size="icon-sm" onClick={() => openEdit(holiday)} title="Edit holiday"><Pencil className="h-4 w-4" /><span className="sr-only">Edit</span></Button>
                                                <Button variant="ghost" size="icon-sm" onClick={() => { setDeleting(holiday); setDeleteOpen(true); }} title="Delete holiday"><Trash2 className="h-4 w-4" /><span className="sr-only">Delete</span></Button>
                                            </div></TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3 border-t px-4 py-3">
                                <p className="text-sm text-muted-foreground">Showing {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, holidays.length)} of {holidays.length} holidays</p>
                                <div className="flex items-center gap-2">
                                    <Button variant="outline" size="sm" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}><ChevronLeft className="mr-1 h-4 w-4" />Previous</Button>
                                    <span className="px-2 text-sm text-muted-foreground">Page {page} of {pageCount}</span>
                                    <Button variant="outline" size="sm" onClick={() => setPage((p) => Math.min(pageCount, p + 1))} disabled={page === pageCount}>Next<ChevronRight className="ml-1 h-4 w-4" /></Button>
                                </div>
                            </div>
                        </>
                    )}
