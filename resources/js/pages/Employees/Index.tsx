...
import { ChevronLeft, ChevronRight, MoreHorizontal, Plus, Search, UserRoundX } from 'lucide-react';
import { Navbar } from '@/components/layout/Navbar';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { EmployeeFormDialog, type Employee } from '@/components/employees/EmployeeFormDialog';
import { FlashMessage } from '@/components/layout/FlashMessage';

const PAGE_SIZE = 15;

type PageProps = { employees: Employee[] };
...
export default function Employees() {
    ...
    const filtered = useMemo(
        () => employees.filter((employee) => employee.full_name.toLowerCase().includes(search.toLowerCase())),
        [employees, search],
    );
    ...
    const toggleMenu = (employee: Employee, event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const openAbove = rect.bottom + menuHeight + gap > window.innerHeight - 8;
        const top = openAbove
    ...
    const activeMenuEmployee = paginatedEmployees.find((employee) => employee.id === menuEmployee);

    return (
        <>
            <FlashMessage />

            <div className="min-h-svh bg-background font-sans" onClick={closeMenu}>
                <Navbar />
                ...
                <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
                    ...
                    <div className="relative z-0 mt-4 overflow-visible rounded-lg border">
                        {filtered.length === 0 ? (
                            <div className="flex flex-col items-center gap-2 p-10 text-center">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {search ? 'Try a different name.' : 'Add your first employee to get started.'}
                                </p>
                                {!search && (
                            ...
                            <>
                                    ...
                                    <table className="w-full caption-bottom text-sm">
                                        ...
                                        <tbody className="[&_tr:last-child]:border-0">
                                            {paginatedEmployees.map((employee) => {
                                                ...
                                                return (
                                                    <tr key={employee.id} className="border-b transition-colors hover:bg-muted/50" onClick={closeMenu}>
                                                        ...
                                                        <td className="whitespace-nowrap p-2 text-right align-middle">
                                                            <Button
                                                                ...
                                                                size="icon"
                                                                className="h-8 w-8"
                                                                onClick={(event) => toggleMenu(employee, event)}
                                                            >
                                ...
                                <div className="flex flex-wrap items-center justify-between gap-3 border-t px-4 py-3">
                                    <p className="text-sm text-muted-foreground">Showing {firstRecord}–{lastRecord} of {filtered.length} employees</p>
                ...
                {activeMenuEmployee && menuPosition && createPortal(
                    <div
                    ...
                    >
                        ...
                        <button type="button" className="flex w-full rounded-sm px-3 py-2 text-left text-sm hover:bg-muted" onClick={() => openEditDialog(activeMenuEmployee)}>
                            Edit
                        </button>
                        <button
                            type="button"
                ...
                {deleteTarget && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm">
                        <div className="w-full max-w-md rounded-[min(var(--radius-4xl),24px)] bg-popover p-6 text-popover-foreground shadow-xl ring-1 ring-foreground/5">
                            <h2 className="text-base font-medium">Delete {deleteTarget.full_name}?</h2>
                            <p className="mt-2 text-sm text-muted-foreground">This removes their employee record. This can't be undone.</p>
