...
class SssContributionService
{
    public function calculate(
        ...
        return [
            'contributionTableId' => $schedule->id,
            'monthlyCompensation' => $this->money($monthlyCompensation),
            'monthlySalaryCredit' => $this->money((float) $schedule->monthly_salary_credit),
            'employeeRegularSs' => $this->money((float) $schedule->employee_regular_ss),
            'employeeMpf' => $this->money((float) $schedule->employee_mpf),
            'employeeTotal' => $this->money((float) $schedule->employee_total),
            'employerRegularSs' => $this->money((float) $schedule->employer_regular_ss),
            'employerMpf' => $this->money((float) $schedule->employer_mpf),
            'employerEc' => $this->money((float) $schedule->employer_ec),
            'employerTotal' => $this->money((float) $schedule->employer_total),
    ...
    private function monthlyCompensation(Employee $employee): float
    {
        if ($employee->rate_type === 'daily') {
            $dailyRate = (float) ($employee->daily_rate ?? $employee->basic_rate ?? 0);
            $divisor = (float) (PayrollSetting::query()->find(1)?->monthly_daily_rate_divisor ?? 26);
            return max(0, $dailyRate * max(1, $divisor));
        }

        return max(0, (float) ($employee->basic_rate ?? 0));
    }
